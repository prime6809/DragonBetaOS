;
; Dragon Beta RAM tester ROM.
;                                        

		use	defsfile

; Config defines, set to 0 to disable, 1 to enable.                                    
UseSRAM		equ	0		; use SRAM on expansion card for stack / vars
UseSerial	equ	1		; Should we echo to serial
FillPageDelay	equ	0		; Delay after filling eatch page in page test     
StopOnPageFail	equ	0		; stop page test on first fail                                 
EmulateSer	equ	0		; Don't wait for serial if emulating!
SetDDEN		equ	1		; set floppy DDEN pin low for setup
...
TestValue	rmb	1		; Current value being used
...
RamOfs		rmb	4		; offset into RAM chip

                     
	org	$c000			; main code
	zmb	$e000-*

; Beginning of ROM so we can check it's length
ROMBegin
...
ScrOutChar
	...
	sta		,x++		; output char, move to next pos
	stx		ScreenPos	; save new screen pos
	puls		x,pc
...
WriteStr
	ifne	UseSerial
	lbsr	SerWriteStr		; Write to serial
	endc
	pshs	d
...
TestPageExists	ldy	SaveScreenPos	; restore screen location
		...
		bne	SizeTestFail	; Non zero reached end of RAM
...
SizeTestFail	puls	a		; restore value from stack
		sta	TestBase	
...
PageTest	clr	FailFlag	; flag success
		...
		clr	DirFlag		; do down test
		bsr	DoPageTest	; do it
		
PageTestEnd	rts

Delay		pshs	x,y		; save regs
		ldx	#0
		ldy	#2
Dloop1		leax	-1,x		; dec counter
		bne	Dloop1		; keep going if not 0
		leay	-1,y		; dec outer counter
		bne	Dloop1		; keep going if not zero
		puls	x,y,pc

DoPageTest	lbsr	SetStartPage	; set starting page
...
FillNext	ldy	SaveScreenPos	; restore screen location
		...
		lbsr	SetStartPage 	; start at page 0
...
TestNextPage	ldy	SaveScreenPos	; restore screen location
		...
		lbsr	SerOutHexByte	; output page number
		
		SerSpace
		
		clr	FailOffset	; make offset 0
		clr	FailOffset+1
		
		lbsr	ShowRAMAbs	; show absolute base of RAM page
		
		SerEOL
		endc
		
		lbsr	ScrOutHexByte	; output page number
...
; Print fail point on serial.....
		ifne	UseSerial	
		pshs	x,d		; save regs		
		leax	FailPageMess,pcr	
		lbsr	SerWriteStrAt
		
		ldd	FailOffset	; get offset
		lbsr	SerOutHexWord
		
		SerSpace
		SerSpace
		
		bsr	ShowRAMAbs	; show absolute offset in RAM
		
		SerSpace
		SerSpace
		
		lda	FailWrote	; get written value
		lbsr	SerOutHexByte
		
		SerSpace
		
		lda	FailRead	; get read value
		lbsr	SerOutHexByte
		
		SerSpace

		lda	FailWrote	; Work out different bits
		eora	FailRead
		lbsr	SerOutHexByte
...
;		sta	FailPage
		...
		ifeq	StopOnPageFail	; should we stop?
		clr	FailFlag	; no clear fail flag
		lda	TestValue
		leax	1,x		; move to next location
		lbra	TestPageLoop	; loop again
		endc
...
ShowRAMAbs
		...
		rora			; shift bottom 4 bits into b
		rorb
		rora
		rorb
		rora
		...
		rora
		rorb
		anda	#$0F		; mask off bits we're not using
		andb	#$F0
		sta	RamOfs		; save in offset
		stb	RamOfs+1		
		
		ldd	FailOffset	; get offset we failed at
		anda	#$0F		; mask out top 4 bits
		ora	RamOfs+1	; combine with pageno
		sta	RamOfs+1	; resave
		stb	RamOfs+2	; save 	
		
		clr	RamOfs+3	; zero a to recieve odd/even bit
		ror	RamOfs		; Divide whole offset by 2 as RAM is 16 bits wide
		ror	RamOfs+1
		ror	RamOfs+2
		ror	RamOfs+3	; get odd/even bit
		
		lda	RamOfs		; output offset into ram
		lbsr	SerOutHexByte

		lda	RamOfs+1	; output offset into ram
		lbsr	SerOutHexByte

		lda	RamOfs+2	; output offset into ram
		lbsr	SerOutHexByte
		
		lda	RamOfs+3	; test high / low
		tsta	
...
StartPageDown	lda	MaxPage		; set start page
		deca
		cmpa	#VidPage	; make sure we're not on vid page if only 128K machine!
		bne	StartPageDown1	; nope set 
		deca			; yes skip vid page
StartPageDown1	sta	CurrentPage			
		rts
...
TestRAM		clr	CurrentPage	; start at page 0
		...
		clr	DirFlag		; so we increment
		
NextPage	leax	TestingMess,pcr	; point to message
		...
		ldx	ScreenPos	; save screen pos
		stx	SaveScreenPos

		leax	TestNCheck55,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		lda	#$55		; checkerboard $55
		lbsr	FillPage	; fill it
		lbsr	TestPage	; test it
		bne	TestFail	; fail it
		
		bsr	RestorePos	; restore screen pos
		leax	TestNCheckAA,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		lda	#$AA		; checkerboard $55
		lbsr	FillPage	; fill it
		lbsr	TestPage	; test it
		bne	TestFail	; fail it

		bsr	RestorePos	; restore screen pos
		leax	TestNWalk1L,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		bsr	TestWalk1L	; do test
		bne	TestFail	; fail it
		
		bsr	RestorePos	; restore screen pos
		leax	TestNWalk1R,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		bsr	TestWalk1R	; do test
		bne	TestFail	; fail it

		bsr	RestorePos	; restore screen pos
		leax	TestNWalk0L,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		bsr	TestWalk0L	; do test
		bne	TestFail	; fail it
		
		bsr	RestorePos	; restore screen pos
		leax	TestNWalk0R,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		bsr	TestWalk0R	; do test
		bne	TestFail	; fail it

		bsr	RestorePos	; restore screen pos
		leax	TestNROMCopy,pcr ; point to message
		lbsr	WriteStr
		SerEOL
		
		bsr	TestROMCopy	; do test
		bne	TestFail	; fail it

		lbsr	TryNextPage	; increment to next page
		lbne	NextPage	; no : continue testing
TestFail		
		rts
		
RestorePos	pshs	y
		ldy	SaveScreenPos	; restore screen location
		sty	ScreenPos	
		puls	y,pc
		

TestWalk1L	lda	#$01		; test $01..$80
TestWalk1LLoop	lbsr	FillPage	; fill it
		lbsr	TestPage	; test it
		bne	TestWalk1LFail	; failed : exit
		
		lsla			; shift value
		tsta			; test for zero
		bne	TestWalk1LLoop	; not there yet do next
		
TestWalk1LFail	rts		

TestWalk1R	lda	#$80		; test $80..$01
TestWalk1RLoop	lbsr	FillPage	; fill it
		lbsr	TestPage	; test it
		bne	TestWalk1RFail	; failed : exit
		
		lsra			; shift value
		tsta			; test for zero
		bne	TestWalk1RLoop	; not there yet do next
		
TestWalk1RFail	rts		

TestWalk0L	lda	#$FE		; test $FE..$7F
TestWalk0LLoop	lbsr	FillPage	; fill it
		lbsr	TestPage	; test it
		bne	TestWalk0LFail	; failed : exit
		
		lsla			; shift value
		ora	$01		; fill in bit
		cmpa	#$FF		; test for end
		bne	TestWalk1LLoop	; not there yet do next
		
TestWalk0LFail	rts		

TestWalk0R	lda	#$7F		; test $7F..$FE
TestWalk0RLoop	lbsr	FillPage	; fill it
		lbsr	TestPage	; test it
		bne	TestWalk0RFail	; failed : exit
...
TestROMCopy	ldx	#TestBase	; point at page
TestRomCopy1	ldy	#ROMBegin	; point at source data

TestRomCopyLoop	
		lda	,y+
		sta	,x+		; save in page
		cmpx	#TestBase+PageSize ; end of page
		beq	ReadROMRAM

		cmpy	#RomEnd		; check for end of rom
		bne	TestRomCopyLoop	; nope keep going
		bra	TestRomCopy1	; re-init rom pointer	

ReadROMRAM	ldx	#TestBase	; point at page
ReadROMRAM1	ldy	#ROMBegin	; point at source data

ReadROMRAMLoop	lda	,y+		; get a byte from ROM
		cmpa	,x		; compare to RAM
		bne	RomTestFail
		
		leax	1,x
		cmpx	#TestBase+PageSize ; end of page
		beq	TestROMCopyEnd	; yep test finished

		cmpy	#RomEnd		; check for end of rom
		bne	ReadROMRAMLoop	; nope keep going
		bra	ReadROMRAM1	; re-init rom pointer	
		
RomTestFail	lbra	PageTestFail

TestROMCopyEnd	tst	FailFlag
		rts		
...
StartTests	lbsr	SerInit		; initialise serial
		leax	RamTestMess,pcr	; point at test message
		lbsr	WriteStrAt	; WriteIt

		ifne	SetDDEN	
		lda	A.DskSel	; get disk select register
		anda	#$ff-B.DblDen	; make dden low.....
		sta 	A.DskSel	
		
		leax	DDENMess,pcr	; point at test message
		lbsr	WriteStrAt	; WriteIt
		endc
		...
		leax	ROMChecksumMess,pcr	; point at test message
		lbsr	WriteStrAt	; WriteIt
		
		clra
		clrb
		ldx	#RomBegin	; point at beginning of ROM
ChecksumLoop		
		addd	,x++		; add to sum
		cmpx	#RomEnd		; end of ROM?
		blo	ChecksumLoop	; nope loop again
		
		lbsr	OutHexWord	; display checksum
...
RamTestMess	FCB	0,0
		FCZ	'Dragon Beta RAM test 2022-04-13, Ramoth'

ROMChecksumMess	FCB	0,1
		FCZ	'ROM checksum : '
...
TestNCheck55	FCZ	' Checkerboard $55'
TestNCheckAA	FCZ	' Checkerboard $AA'
TestNWalk1L	FCZ	' Walking 1 left  '
TestNWalk1R	FCZ	' Walking 1 right '
TestNWalk0L	FCZ	' Walking 0 left  '
TestNWalk0R	FCZ	' Walking 0 right '
TestNROMCopy	FCZ	' ROM copy data   '
...
; End of ROM so we can check it's length
ROMEnd
